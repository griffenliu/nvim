...
_M.setup = function()
    ...
    -- ### Libs
    local libs_after = require("plugins.libs").setup()

    -- FIXME: 最好可以在这里有一个阻挡，当基础库加载完毕再开始其他插件加载，虽然时间可能会慢一些
    -- ### 主题
    local theme_tokyonight = require('plugins.configs.themes.tokyonight'):after(libs_after)
    ...
    -- ### LSP
    -- 安装工具
    local lsp_installer = require('plugins.configs.lsp.installer'):after(ide_telescope)
    -- 常用配置
    local lsp_config = require('plugins.configs.lsp.config'):after(lsp_installer)
    -- UI美化
    local lsp_kind = require('plugins.configs.lsp.lspkind'):after(lsp_config)
    -- 提供代码诊断，格式化等
    require('plugins.configs.lsp.null-ls'):after(lsp_config)
    -- 诊断进度等
    require('plugins.configs.lsp.loading'):after(lsp_config)
    -- Actions
    require('plugins.configs.lsp.lspsaga'):after(lsp_config)
    -- 虚拟行切换
    require('plugins.configs.lsp.virtual-line'):after(lsp_config)

    -- ### 补全
    local cmp_cmp = require('plugins.configs.cmp.cmp'):after(lsp_kind)
    -- 代码片段引擎
    local cmp_snip = require('plugins.configs.cmp.snip'):after(cmp_cmp)
    ...
    local cmp_source_lsp = require('plugins.configs.cmp.sources.lsp'):after(cmp_cmp)
    local cmp_source_buffer = require('plugins.configs.cmp.sources.buffer'):after(cmp_cmp)
    local cmp_source_path = require('plugins.configs.cmp.sources.path'):after(cmp_cmp)
    local cmp_source_nvim_lua = require('plugins.configs.cmp.sources.nvim-lua'):after(cmp_cmp)
