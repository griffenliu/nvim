...
local _M = base.new {
  ...
  packer = {
    ...
    config = function()
      require("plugins.configs.lsp.virtual-line"):config()
    end
  }
}
...
local function toggle_virtual_line()
  if is_virtual_text then
    diagnostic.config({ virtual_text = false, virtual_lines = true })
    is_virtual_text = false
  else
    diagnostic.config({ virtual_text = { prefix = "" }, virtual_lines = false })
