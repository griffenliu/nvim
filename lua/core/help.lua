...
local function create_help_buffer()
  ...
  -- 在这个缓冲区设置一个退出按键
  api.nvim_buf_set_keymap(help_bufnr, "n", "q", "<cmd> ToggleHelp <CR>", { silent = true, noremap = true })
...
local function hide_help_window(wid, bufid)
  ...
  if wid and api.nvim_win_is_valid(wid) then
    vim.api.nvim_win_close(wid, { force = true })
  end
end
...
_M.setup = function()
  ...
  vim.api.nvim_set_keymap("n", "<leader>hh", "<cmd> ToggleHelp <CR>", { noremap = true,
    silent = true, nowait = true, expr = false, desc = "@C [HELP] 显示我的帮助" })
end
